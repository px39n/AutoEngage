...
class Video:
    ...
    def Clip(self, Frame_address):
        ...
        for subdir in tqdm(subdirectories):
            ...
            with open(os.path.join(subdir, 'Bounding_Box.json'), 'r') as infile:
                bounding_box = json.load(infile)

            # Get a list of all the .jpg files in the subdirectory
            images = [f.path for f in os.scandir(subdir) if f.is_file() and f.name.endswith('.jpg')]
        ...
        print("Images cropped according to bounding boxes and saved.")
